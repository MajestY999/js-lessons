...
for(let i = 1; i <= 20; i++ ){
 if(i == 2 || i == 3 || i == 4){
    console.log(`Число увеличилось в ${i} раза`);
 } else {
    console.log(`Число увеличилось в ${i} раз`);
 }
}

// let arr3 = [1, 2, 3, 4];
// for (let element  of arr3) {
//   console.log(element); // Выводит значения элементов: 1, 2, 3, 4
// }
...
const reslut = prices   //  Тут мы передаем то что наша переменна результат const reslut = prices   //  Тут мы передаем то что наша переменна результат равна ценам и тогда в будущем мы будет выводить основной и  уже как раз измененй массив 

.map(
    price => price[1] > price[0]
)
.filter(
    price => price > 0 //Тут мы же как раз обозначаем что в нутри нашего массива не должно быть никаких чисел которые меньше нуля.
)

console.log(reslut);

// Берёт каждый элемент массива prices — то есть [100, 200], [120, 100], [200, 350].

// Для каждого элемента считает разницу:
